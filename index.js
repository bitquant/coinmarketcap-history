'use strict';
const puppeteer = require('puppeteer');

function getTextContent(element) {
    return element.getProperty('textContent').then(property => property.jsonValue());;
}

const getHistory = async (currency) => {

    const browser = await puppeteer.launch({args: ["--no-sandbox"]});
    const page = await browser.newPage();

    await page.setUserAgent("Mozilla/5.0 (Macintosh; Intel Mac OS X 10_11_6) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/63.0.3205.0 Safari/537.36");
    await page.setRequestInterception(true);

    page.on('request', (request) => {

        if ((request.resourceType() === 'image') || (request.resourceType() === 'stylesheet')
            || (request.resourceType() === 'font') || (request.resourceType() === 'media')) {
            request.abort();
        }
        else if ((request.resourceType() === 'script') || (request.resourceType() === 'xhr')) {
            if (request.url().search(/:\/\/.*coinmarketcap\.com\//) !== -1) {
                //console.log("ALLOW: " + request.resourceType() + "@" + request.url());
                request.continue();
            }
            else {
                //console.log("BLOCK: " + request.resourceType() + "@" + request.url());
                request.abort();
            }
        }
        else if (request.resourceType() === 'document') {
            //console.log("ALLOW: " + request.resourceType() + "@" + request.url());
            request.continue();
        }
        else {
            //console.log("BLOCK: " + request.resourceType() + "@" + request.url());
            request.abort();
        }
    });

    //page.on('console', msg => console.log('PAGE_LOG:', msg.text));

    try {
        const rsp = await page.goto(`https://coinmarketcap.com/currencies/${currency}/historical-data/?start=20080204&end=20280104`);

        const selector = '#historical-data table tbody tr';
        await page.waitForSelector(selector, { timeout: 9000 });

        let historyData = await page.evaluate(function(currency) {           // Line 1
            var historyRoot = document.getElementById("historical-data");    // Line 2
            var tableRoot = historyRoot.getElementsByClassName("table");     // Line 3
            var tableBody = tableRoot[0].getElementsByTagName("tbody");      // ....
            var tableRows = tableBody[0].getElementsByTagName("tr");

            var historyArray = [];

            for (var idx = 0; idx < tableRows.length; idx++) {
               var tableData = tableRows[idx].getElementsByTagName("td");
               var historyEntry = [];
               for (var jdx = 0; jdx < tableData.length; jdx++) {
                   historyEntry.push(tableData[jdx].textContent);
               }
               historyArray.push(historyEntry);
            }

            return historyArray;
        }, currency);

        browser.close();
        return historyData;

        //
        // The code below is a little cleaner but executes 10x slower
        //

        /*
        const tableRowElements = await page.$$(selector);
        const historyArray = [];

        for (let row of tableRowElements) {
            const rowData = await row.$$("td");
            const historyEntry = [];
            for (let field of rowData) {
                historyEntry.push(await getTextContent(field));
            }
            historyArray.push(historyEntry);
        }

        browser.close();
        return historyArray;
        */
    }
    catch (err) {
        console.error(err.stack ? err.stack : err)
    }

    browser.close();
    throw new Error(`unable to fetch data for ${currency}`);
};

exports.getHistory = getHistory;
